...
begin
  ...
  namespace :go do
    ...
    task :build do
      ...
      sh "cd go && CGO_ENABLED=1 go build -buildmode=c-archive -o build/#{platform}/libbubblezone.a ."
    end
    ...
    task :build_all do
      GO_PLATFORMS.each_value do |env|
        ...
        FileUtils.mkdir_p(output_dir)
        sh "cd go && CGO_ENABLED=1 GOOS=#{env[:goos]} GOARCH=#{env[:goarch]} go build -buildmode=c-archive -o build/#{env[:goos]}_#{env[:goarch]}/libbubblezone.a ."
      end
    end
  ...
  Rake::ExtensionTask.new do |ext|
    ext.name = "bubblezone"
    ext.ext_dir = "ext/bubblezone"
    ext.lib_dir = "lib/bubblezone"
    ext.source_pattern = "*.c"
    ext.gem_spec = Gem::Specification.load("bubblezone.gemspec")
  ...
  namespace "gem" do
    task "prepare" do
      ...
      gemspec_path = File.expand_path("./bubblezone.gemspec", __dir__)
    ...
    PLATFORMS.each do |platform|
      ...
      task platform => "prepare" do
        ...
        build_script = <<~BASH
          ...
          CGO_ENABLED=1 CC=#{env[:cc]} GOOS=#{env[:goos]} GOARCH=#{env[:goarch]} go build -buildmode=c-archive -o build/#{env[:goos]}_#{env[:goarch]}/libbubblezone.a . && \
          cd .. && \
          rm -f .ruby-version && \
          rm -rf vendor/bundle && \
          bundle install && \
